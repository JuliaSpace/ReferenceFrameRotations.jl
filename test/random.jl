...
@testset "Euler Angle and Axis (Float64)" begin
    aa_exp = EulerAngleAxis(
        3.0607810768487105,
        @SVector [-0.0748748188948659, -0.9810600691052008, -0.17864742456234697]
    )
...
@testset "Euler Angle and Axis (Float32)" begin
    aa_exp = EulerAngleAxis(1.4419688f0, @SVector [0.056824237f0, 0.812369f0, -0.5803686f0])
...
@testset "Euler Angles (Float64)" begin
    ea_exp = EulerAngles(6.237765228608508, 2.646507063149075, 4.53226625968865, :YZY)
...
@testset "Euler Angles (Float32)" begin
    ea_exp = EulerAngles(2.7291467f0, 1.5209f0, 2.1911314f0, :YZY)
...
@testset "Quaternion (Float64)" begin
    q_exp = Quaternion(
        0.04039479466622753, -0.07481370585716751, -0.9802593251184702, -0.17850161203213802
    )
...
@testset "Quaternion (Float32)" begin
    q_exp = Quaternion(0.7511563f0, 0.037511066f0, 0.5362646f0, -0.38311547f0)
...
@testset "CRP (Float64)" begin
    c_exp = CRP(-1.8520630312725974, -24.266971356535347, -4.418926089538365)
...
@testset "CRP (Float32)" begin
    c_exp = CRP(0.049937766f0, 0.7139188f0, -0.5100343f0)
...
@testset "MRP (Float64)" begin
    m_exp = MRP(-0.07190895825384126, -0.9421993748372708, -0.17157103529089043)
...
@testset "MRP (Float32)" begin
    m_exp = MRP(0.021420741f0, 0.30623457f0, -0.21877857f0)
