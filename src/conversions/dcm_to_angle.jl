...
function dcm_to_angle(dcm::DCM{T}, rot_seq::Symbol = :ZYX) where {T <: Number}
    if rot_seq == :ZYX
        ...
        if !(abs(dcm[1, 3]) ≥ 1 - eps(T))
            ...
            return EulerAngles(
                _mod_atan(-dcm[2, 1], +dcm[2, 2]), _mod_asin(-dcm[1, 3]), T(0), rot_seq
            )
        end
    elseif rot_seq == :XYX
        # Check for singularities.
        if !(abs(dcm[1, 1]) ≥ 1 - eps(T))
            return EulerAngles(
            ...
            return EulerAngles(
                _mod_atan(+dcm[2, 3], +dcm[2, 2]), _mod_acos(dcm[1, 1]), T(0), rot_seq
            )
        end
    elseif rot_seq == :XYZ
        # Check for singularities.
        if !(abs(dcm[3, 1]) ≥ 1 - eps(T))
            return EulerAngles(
            ...
            return EulerAngles(
                _mod_atan(+dcm[2, 3], +dcm[2, 2]), _mod_asin(+dcm[3, 1]), T(0), rot_seq
            )
        end
    elseif rot_seq == :XZX
        # Check for singularities.
        if !(abs(dcm[1, 1]) ≥ 1 - eps(T))
            ...
            return EulerAngles(
                _mod_atan(-dcm[3, 2], +dcm[3, 3]), _mod_acos(dcm[1, 1]), T(0), rot_seq
            )
        end
    elseif rot_seq == :XZY
        # Check for singularities.
        if !(abs(dcm[2, 1]) ≥ 1 - eps(T))
            return EulerAngles(
            ...
            return EulerAngles(
                _mod_atan(-dcm[3, 2], +dcm[3, 3]), _mod_asin(-dcm[2, 1]), T(0), rot_seq
            )
        end
    elseif rot_seq == :YXY
        # Check for singularities.
        if !(abs(dcm[2, 2]) ≥ 1 - eps(T))
            return EulerAngles(
            ...
            return EulerAngles(
                _mod_atan(-dcm[1, 3], +dcm[1, 1]), _mod_acos(dcm[2, 2]), T(0), rot_seq
            )
        end
    elseif rot_seq == :YXZ
        if !(abs(dcm[3, 2]) ≥ 1 - eps(T))
            return EulerAngles(
            ...
            return EulerAngles(
                _mod_atan(-dcm[1, 3], +dcm[1, 1]), _mod_asin(-dcm[3, 2]), T(0), rot_seq
            )
        end
    elseif rot_seq == :YZX
        # Check for singularities.
        if !(abs(dcm[1, 2]) ≥ 1 - eps(T))
            return EulerAngles(
            ...
            return EulerAngles(
                _mod_atan(+dcm[3, 1], +dcm[3, 3]), _mod_asin(+dcm[1, 2]), T(0), rot_seq
            )
        end
    elseif rot_seq == :YZY
        # Check for singularities.
        if !(abs(dcm[2, 2]) ≥ 1 - eps(T))
            ...
            return EulerAngles(
                _mod_atan(dcm[3, 1], dcm[3, 3]), _mod_acos(dcm[2, 2]), T(0), rot_seq
            )
        end
    elseif rot_seq == :ZXY
        # Check for singularities.
        if !(abs(dcm[2, 3]) ≥ 1 - eps(T))
            return EulerAngles(
            ...
            return EulerAngles(
                _mod_atan(+dcm[1, 2], +dcm[1, 1]), _mod_asin(+dcm[2, 3]), T(0), rot_seq
            )
        end
    elseif rot_seq == :ZXZ
        # Check for singularities.
        if !(abs(dcm[3, 3]) ≥ 1 - eps(T))
            return EulerAngles(
            ...
            return EulerAngles(
                _mod_atan(dcm[1, 2], dcm[1, 1]), _mod_acos(dcm[3, 3]), T(0), rot_seq
            )
        end
    elseif rot_seq == :ZYZ
        # Check for singularities.
        if !(abs(dcm[3, 3]) ≥ 1 - eps(T))
            ...
            return EulerAngles(
                _mod_atan(-dcm[2, 1], dcm[2, 2]), _mod_acos(dcm[3, 3]), T(0), rot_seq
            )
        end
    else
